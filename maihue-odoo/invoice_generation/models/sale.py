...
class SaleOrder(models.Model):
        ...
        for line in lines:
            if lines_grouped:
                for lg in lines_grouped:
                    if 'sale_line_ids' in line:
                        order = SaleOrderLine.browse(line['sale_line_ids'][0][1]).order_id
                    if line['product_id'] == lg['product_id'] and line['price_unit'] == lg['price_unit'] and order.cost_center_id == lg['cost_center_str']:
                        name = lg['name']
                        # preiodo_mes = order.periodo_mes.split('/')
                        # mes = preiodo_mes[0][0:3]
                        # anio = preiodo_mes[1][2:4]
                        # name += ' ' + mes + anio if order else ''
    ...
    def references_invoice_oc_rental(self, lines_grouped):
        references = []
        create_references = []
        for line in lines_grouped:
            ...
            if 'sale_line_ids' in line:
                for l in line['sale_line_ids']:
                    sale_lines.append(l[1])
            sale_line_ids = self.env['sale.order.line'].search([('id', 'in', sale_lines)])
            sale_orders = sale_line_ids.mapped('order_id')
            for order in sale_orders:
                if order.reference_ids:
                    if references:
                        create_references = []
                        for reference in order.reference_ids:
                            add = True
                            for ref in references:
                                if reference.origin_doc_number == ref[2]['origin_doc_number']:
                                    add = False
                                    break
                            if add:
                                create_references.append(reference)
                    else:
                        create_references = order.reference_ids
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        if self.agreement_id:
            ...
            for inv in invoice_vals_list:
                ...
                ###### INICIO ORGANIZANDO LOS PERIODOS ##########
                ...
                for line in grouped_invoice_lines:
                    ...
                    if 'sale_line_ids' in line:
                        for l in line['sale_line_ids']:
                            sale_lines.append(l[1])
                    sale_lines.sort()
                    for ol in sale_lines:
                        order_line = SaleOrderLine.browse([ol])
                        if order_line.order_id.tipo_rental_order == 'mensualidad':
                            date_start = order_line.order_id.inicio_fecha_alquiler
                            mes = months[date_start.month - 1]
                            anio = datetime.strftime(date_start, '%Y')[2:4]
                            name += ' ' + mes + anio
                        else:
                            name += ' ' + order_line.order_id.tipo_rental_order
                    line['name'] += name
                    ###### FIN ORGANIZANDO LOS PERIODOS ##########
                    create_invoice_lines.append((0, 0, line))
            inv['invoice_line_ids'] = create_invoice_lines
            references = self.references_invoice_oc_rental(grouped_invoice_lines)
            inv['l10n_cl_reference_ids'] = references
    ...
    def _prepare_invoice(self):
        ...
        res['invoice_partner_dir_id'] = self.agreement_id.partner_invoice_id.id if self.agreement_id and self.agreement_id.partner_invoice_id else False
        res['pre_liquid'] = self.pre_liquid
        ...
        res['scheduled_date'] = self._context.get('scheduled_date', False)
        res['invoice_date'] = self.scheduled_date
        res['mass_invoice'] = self.mass_invoice
        res['uf_rate'] = self.uf_rate
        document_type = self.env['l10n_latam.document.type'].search([('code', '=', '33')]) if self.partner_id.company_type == 'company' else self.env['l10n_latam.document.type'].search([('code', '=', '39')])
        if document_type:
...
class SaleOrderLine(models.Model):
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if self.order_id.agreement_id:
            ...
            # if self.order_id.tipo_rental_order == 'mensualidad':
            #     preiodo_mes = self.order_id.periodo_mes.split('/')
            #     mes = preiodo_mes[0][0:3]
            #     anio = preiodo_mes[1][2:4]
            #     date_range = mes + anio
            # else:
            #     date_range = self.order_id.tipo_rental_order
            res['cost_center_str'] = self.order_id.cost_center_id
