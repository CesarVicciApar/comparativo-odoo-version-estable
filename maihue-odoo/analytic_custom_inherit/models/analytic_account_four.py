...
class AccountAnalyticDistributionFour(models.Model):
    _name = 'account.analytic.distribution.four'
    _description = 'Analytic Account Distribution 4'
    _rec_name = 'account_id'

    account_id = fields.Many2one('account.analytic.account.four', string='Analytic Account', required=True)
    ...
    tag_id = fields.Many2one('account.analytic.tag.four', string="Parent tag", required=True)
...
class AccountAnalyticTagFour(models.Model):
    _name = 'account.analytic.tag.four'
    _description = 'Analytic Tags 4'
    ...
    analytic_distribution_ids = fields.One2many('account.analytic.distribution.four', 'tag_id', string="Analytic Accounts")
    company_id = fields.Many2one('res.company', string='Company')


class AccountAnalyticGroupFour(models.Model):
    _name = 'account.analytic.group.four'
    _description = 'Analytic Categories 4'
    _parent_store = True
    ...
    children_ids = fields.One2many('account.analytic.group.four', 'parent_id', string="Childrens")
...
class AccountAnalyticAccountFour(models.Model):
    _name = 'account.analytic.account.four'
    _inherit = ['mail.thread']
    _description = 'Analytic Account 4'
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        """
        res = super(AccountAnalyticAccountFour, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
        accounts = self.env['account.analytic.account.four']
        for line in res:
            if '__domain' in line:
    ...
    def _compute_debit_credit_balance(self):
        ...
        analytic_line_obj = self.env['account.analytic.line.four']
        domain = [
    ...
    name = fields.Char(string='Analytic Account 4', index=True, required=True, tracking=True)
    ...
    group_id = fields.Many2one('account.analytic.group.four', string='Group', check_company=True)

    line_ids = fields.One2many('account.analytic.line.four', 'account_id', string="Analytic Lines")

    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)

    # use auto_join to speed up
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        if operator not in ('ilike', 'like', '=', '=like', '=ilike', 'not ilike'):
            return super(AccountAnalyticAccountFour, self)._name_search(name, args, operator, limit, name_get_uid=name_get_uid)
        args = args or []
...
class AccountAnalyticLineFour(models.Model):
    _name = 'account.analytic.line.four'
    _description = 'Analytic Line 4'
    ...
    account_id = fields.Many2one('account.analytic.account.four', 'Analytic Account', required=True, ondelete='restrict', index=True, check_company=True)
    partner_id = fields.Many2one('res.partner', string='Partner', check_company=True)
    user_id = fields.Many2one('res.users', string='User', default=_default_user)
    tag_ids = fields.Many2many('account.analytic.tag.four', 'account_analytic_line_four_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True, check_company=True)
    ...
    group_id = fields.Many2one('account.analytic.group.four', related='account_id.group_id', store=True, readonly=True, compute_sudo=True)
    ...
    def view_header_get(self, view_id, view_type):
        if self.env.context.get('account_id'):
            return _(
                "Entries: %(account)s",
                account=self.env['account.analytic.account.four'].browse(self.env.context['account_id']).name
            )
