...
class AccountAnalyticDistributionTwo(models.Model):
    _name = 'account.analytic.distribution.two'
    _description = 'Analytic Account Distribution 2'
    _rec_name = 'account_id'

    account_id = fields.Many2one('account.analytic.account.two', string='Analytic Account', required=True)
    ...
    tag_id = fields.Many2one('account.analytic.tag.two', string="Parent tag", required=True)
...
class AccountAnalyticTagTwo(models.Model):
    _name = 'account.analytic.tag.two'
    _description = 'Analytic Tags 2'
    ...
    analytic_distribution_ids = fields.One2many('account.analytic.distribution.two', 'tag_id', string="Analytic Accounts")
    company_id = fields.Many2one('res.company', string='Company')

class AccountAnalyticGroupTwo(models.Model):
    _name = 'account.analytic.group.two'
    _description = 'Analytic Categories 2'
    _parent_store = True
    ...
    children_ids = fields.One2many('account.analytic.group.two', 'parent_id', string="Childrens")
...
class AccountAnalyticAccountTwo(models.Model):
    _name = 'account.analytic.account.two'
    _inherit = ['mail.thread']
    _description = 'Analytic Account 2'
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        """
        res = super(AccountAnalyticAccountTwo, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
        accounts = self.env['account.analytic.account.two']
        for line in res:
            if '__domain' in line:
    ...
    def _compute_debit_credit_balance(self):
        ...
        analytic_line_obj = self.env['account.analytic.line.two']
        domain = [
    ...
    name = fields.Char(string='Analytic Account 2', index=True, required=True, tracking=True)
    ...
    group_id = fields.Many2one('account.analytic.group.two', string='Group', check_company=True)

    line_ids = fields.One2many('account.analytic.line.two', 'account_id', string="Analytic Lines")
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        if operator not in ('ilike', 'like', '=', '=like', '=ilike', 'not ilike'):
            return super(AccountAnalyticAccountTwo, self)._name_search(name, args, operator, limit, name_get_uid=name_get_uid)
        args = args or []
...
class AccountAnalyticLineTwo(models.Model):
    _name = 'account.analytic.line.two'
    _description = 'Analytic Line 2'
    ...
    account_id = fields.Many2one('account.analytic.account.two', 'Analytic Account', required=True, ondelete='restrict', index=True, check_company=True)
    partner_id = fields.Many2one('res.partner', string='Partner', check_company=True)
    user_id = fields.Many2one('res.users', string='User', default=_default_user)
    tag_ids = fields.Many2many('account.analytic.tag.two', 'account_analytic_line_two_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True, check_company=True)
    ...
    group_id = fields.Many2one('account.analytic.group.two', related='account_id.group_id', store=True, readonly=True, compute_sudo=True)
