...
class AccountAnalyticDistributionThree(models.Model):
    _name = 'account.analytic.distribution.three'
    _description = 'Analytic Account Distribution 3'
    _rec_name = 'account_id'

    account_id = fields.Many2one('account.analytic.account.three', string='Analytic Account', required=True)
    ...
    tag_id = fields.Many2one('account.analytic.tag.three', string="Parent tag", required=True)
...
class AccountAnalyticTagThree(models.Model):
    _name = 'account.analytic.tag.three'
    _description = 'Analytic Tags 3'
    ...
    analytic_distribution_ids = fields.One2many('account.analytic.distribution.three', 'tag_id', string="Analytic Accounts")
    company_id = fields.Many2one('res.company', string='Company')

class AccountAnalyticGroupThree(models.Model):
    _name = 'account.analytic.group.three'
    _description = 'Analytic Categories 3'
    _parent_store = True
    _rec_name = 'complete_name'

    name = fields.Char(required=True)
    description = fields.Text(string='Description')
    parent_id = fields.Many2one('account.analytic.group.three', string="Parent", ondelete='cascade', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    parent_path = fields.Char(index=True)
    children_ids = fields.One2many('account.analytic.group.three', 'parent_id', string="Childrens")
...
class AccountAnalyticAccountThree(models.Model):
    _name = 'account.analytic.account.three'
    _inherit = ['mail.thread']
    _description = 'Analytic Account 3'
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        """
        res = super(AccountAnalyticAccountThree, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
        accounts = self.env['account.analytic.account.three']
        for line in res:
            if '__domain' in line:
    ...
    def _compute_debit_credit_balance(self):
        ...
        analytic_line_obj = self.env['account.analytic.line.three']
        domain = [
    ...
    group_id = fields.Many2one('account.analytic.group.three', string='Group', check_company=True)

    line_ids = fields.One2many('account.analytic.line.three', 'account_id', string="Analytic Lines")
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        if operator not in ('ilike', 'like', '=', '=like', '=ilike', 'not ilike'):
            return super(AccountAnalyticAccountThree, self)._name_search(name, args, operator, limit, name_get_uid=name_get_uid)
        args = args or []
...
class AccountAnalyticLineThree(models.Model):
    _name = 'account.analytic.line.three'
    ...
    account_id = fields.Many2one('account.analytic.account.three', 'Analytic Account', required=True, ondelete='restrict', index=True, check_company=True)
    partner_id = fields.Many2one('res.partner', string='Partner', check_company=True)
    user_id = fields.Many2one('res.users', string='User', default=_default_user)
    tag_ids = fields.Many2many('account.analytic.tag.three', 'account_analytic_line_three_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True, check_company=True)
    ...
    group_id = fields.Many2one('account.analytic.group.three', related='account_id.group_id', store=True, readonly=True, compute_sudo=True)
    category = fields.Selection([('other', 'Other'),('invoice', 'Customer Invoice'), ('vendor_bill', 'Vendor Bill')], default='other')
    ...
    def view_header_get(self, view_id, view_type):
        if self.env.context.get('account_id'):
            return _(
                "Entries: %(account)s",
                account=self.env['account.analytic.account.two'].browse(self.env.context['account_id']).name
            )
