...
class AgreementExtra(models.Model):
    ...
    _inherit = ['mail.render.mixin', 'mail.thread', 'mail.activity.mixin']
    _description = "Agreement Extra"
    _order = "sequence"
    _AGREEMENT_EXTRA_FIELDS = ['content', 'dynamic_content']

    def _compute_partner_repres_ids_domain(self):
        for partner in self:
            partners = False
            if partner.partner_id:
                if partner.partner_id.company_type == 'company':
                    partners = self.env['res.partner'].search(
                        [('parent_id', '=', partner.partner_id.id), ('repres_legal', '=', True)])
                    if partners:
                        partner.partner_repres_ids_domain = partners.ids
            else:
                partner.partner_repres_ids_domain = (0, 0, '')
    ...
    required_sign = fields.Boolean(
        ...
        default=False)
    send_document_automatic = fields.Boolean(
        string="Enviar documento automatico",
        default=False)
    status_send_document = fields.Selection(
        string='Estado Envio Documento',
        selection=[('not_send', 'No enviado'),
                   ('send', 'Enviado'),
    ...
    firma = fields.Selection([
        ...
        ('fisica', 'Fisica'),
    #    ('ambas', 'Ambas')
    ], string="Firma", default="digital", track_visibility="onchange")
    ...
    partner_repres_ids_domain = fields.Many2many("res.partner", string="Partner")#, compute=_compute_partner_repres_ids_domain)
    ...
    state_firm1 = fields.Selection(selection=[
        ...
        ('T', 'Pend. Firma'),
        ('S', 'Firma en Revisión'),
        ('cancelled', 'Cancelado')], string='Estado Firma R1', track_visibility="onchange")
    ...
    state_firm2 = fields.Selection(selection=[
        ...
        ('T', 'Pend. Firma'),
        ('S', 'Firma en Revisión'),
        ('cancelled', 'Cancelado')], string='Estado Firma R2', track_visibility="onchange")
    ...
    state_firm3 = fields.Selection(selection=[
        ...
        ('T', 'Pend. Firma'),
        ('S', 'Firma en Revisión'),
        ('cancelled', 'Cancelado')], string='Estado Firma R3', track_visibility="onchange")
    company_signed_date = fields.Date(
        string="Fecha Firma Maihue",
        track_visibility="onchange",
    ...
    state_firm4 = fields.Selection(selection=[
        ...
        ('T', 'Pend. Firma'),
        ('S', 'Firma en Revisión'),
        ('cancelled', 'Cancelado')], string='Estado Firma', track_visibility="onchange")
    signed_contract = fields.Binary(
        string="Documento Firmado", track_visibility="onchange")
    firma_type = fields.Many2one('signature.type', string="Tipo de Firma", track_visibility="onchange")

    @api.onchange('partner_signed_user_id')
    def onchange_partner_signed_user_id(self):
        if self.partner_signed_user_id:
            self.email_signed_user = self.partner_signed_user_id.email
        else:
            self.email_signed_user = False
            self.fecha_repres1 = False
            self.fecha_envio1 = False
            self.state_firm1 = False

    @api.onchange('company_signed_user_dos_id')
    def onchange_company_signed_user_dos_id(self):
        if self.company_signed_user_dos_id:
            self.email_signed_user_dos = self.company_signed_user_dos_id.email
        else:
            self.email_signed_user_dos = False
            self.state_firm2 = False
            self.fecha_envio2 = False
            self.fecha_repres2 = False

    @api.onchange('company_signed_user_tres_id')
    def onchange_company_signed_user_tres_id(self):
        if self.company_signed_user_tres_id:
            self.email_signed_user_tres = self.company_signed_user_tres_id.email
        else:
            self.email_signed_user_tres = False
            self.state_firm3 = False
            self.fecha_envio3 = False
            self.fecha_repres3 = False

    @api.onchange('company_signed_user_id')
    def onchange_company_signed_user_id(self):
        if self.company_signed_user_id:
            self.email_signed_maihue = self.company_signed_user_id.email
        else:
            self.email_signed_maihue = False
    ...
    def _compute_dynamic_content(self):
        for record in self:
            if record.content:
                ...
                agreement_extra_id = record.with_context(lang=context)
                try:
                    mail_values = agreement_extra_id.with_context(template_preview_lang=context).generate_email(
                        [record.id], record._AGREEMENT_EXTRA_FIELDS)
